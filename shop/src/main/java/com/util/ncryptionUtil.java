...
public class ncryptionUtil {
    ...
    public static int comparePassword1(String password, SysAuthority sysAuthority){
        // 将用户名作为盐值
        ByteSource salt = ByteSource.Util.bytes(sysAuthority.getUsername());
        //将MD5加密后的密文与数据库中的密文对比,如果正确返回1
        String newPs = new SimpleHash("MD5", password, salt, 1024).toHex();
        if (newPs.equals(sysAuthority.getPassword())){
            return 1;
        }
        return 0;
    }

    public static String comparePassword1(String salt,String password){
        String newPs = new SimpleHash("MD5",password,salt,1024).toHex();
        return newPs;
    }
